...
export default function NavbarUI({
  ...
  // Helper: build query param URL from area slug
  const areaHref = (area: any) => `/tours?city=${area.slug?.current ?? area.slug}`;
  ...
  return (
    <div className="sticky top-0 z-50 bg-gradient-to-r from-[#0f172a] to-[#1e293b] text-white font-serif">
      ...
      <div className="hidden xl:flex items-center justify-center flex-nowrap gap-8 px-6 py-1 bg-gradient-to-r from-[#0f172a] to-[#1e293b] font-medium relative whitespace-nowrap">
          ...
          {activeMenu === "india" && (
            <>
              ...
              <div className="absolute left-0 top-full flex bg-white text-black shadow-2xl rounded z-50">
                ...
                {/* AREAS — ✅ FIXED: query param instead of dynamic route */}
                {selectedState && (
                  <div className="w-[300px] bg-white">
                    <div className="px-4 py-3 font-semibold border-b">
                      {selectedState.name}
        ...
        <Link href="/blog" className="cursor-pointer hover:text-yellow-300 py-2 transition">
          Blog
        </Link>

        <Link href="/contact" className="cursor-pointer hover:text-yellow-300 py-2 transition">
          Contact Us
        </Link>
          ...
          <div className="p-4 space-y-6 text-[15px]">
                ...
                <div className="pl-3 mt-2 border-l space-y-2">
                  {indiaStates.map((state: any) => (
                    <div key={state._id}>
                      ...
                      {openMenu[state._id] && (
                        <div className="pl-3 mt-1 border-l text-sm text-gray-500 space-y-1">
                          {/* ✅ FIXED: mobile India areas now also use query params */}
                          {(areasByState[state._id] || []).map((area: any) => (
                            <Link
                              key={area._id}
                              href={areaHref(area)}
                        ...
                        <div className="pl-3 mt-1 border-l space-y-2">
                          {(statesByCountry[country._id] || []).map((state: any) => (
                            <div key={state._id}>
                              ...
                              {openMenu[state._id] && (
                                <div className="pl-3 mt-1 border-l text-sm text-gray-500 space-y-1">
                                  {/* ✅ FIXED: mobile World areas now also use query params */}
                                  {(areasByState[state._id] || []).map((area: any) => (
                                    <Link
                                      key={area._id}
                                      href={areaHref(area)}
                  ...
                  {specialTours?.slice(0, 4).map((t: any) => (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-500">{t.count} Departures</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="pt-2 border-t space-y-3 flex flex-col">
              <Link
                href="/blog"
                onClick={() => setOpen(false)}
                className="hover:text-yellow-600 py-1 transition"
              >
                Blog
              </Link>
              <Link
