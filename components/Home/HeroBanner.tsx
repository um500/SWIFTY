            ...
            data.map((item, i) => (
              ...
              <Link
                ...
                className={`flex-1 flex flex-col items-center justify-center py-4 md:py-6 px-3 text-center border-b md:border-b-0 md:border-r border-gray-600 last:border-r-0 transition-all duration-300 ${
                  activeTab === i ? "bg-[#334155]" : "hover:bg-[#273449]"
                }`}
              >
                <div className="text-lg md:text-2xl font-bold text-yellow-400">
                  ₹{item.tour?.price?.toLocaleString("en-IN")}
                </div>
                <div className="w-8 h-[1px] bg-gray-500 my-2" />
                <div className="text-sm md:text-base font-medium">{item.tour?.title}</div>
                <div className="text-xs text-gray-300">({item.tour?.days})</div>
              </Link>
            ))
          ) : (
            <div className="p-6 text-center text-gray-300 w-full">No tours available</div>
          )}
        </div>
      </div>
    </section>
