// ============================================
// 📁 components/home/FeaturedTour.tsx
...
const FeaturedTour = () => {
  ...
  useEffect(() => {
    ...
    const onEnd = () => {
      ...
      if (cur >= total + 1) { jumpTo(1); setDotIndex(0); }
      else if (cur <= 0)    { jumpTo(total); setDotIndex(total - 1); }
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => { jumpTo(1); setDotIndex(0); });
  }, [banners.length, jumpTo]);
  ...
  const startAutoPlay = useCallback(() => {
    stopAutoPlay();
    autoPlayRef.current = setInterval(() => slideTo(currentRef.current + 1), 3500);
  ...
  return (
    <section className="w-full bg-white">
      <div className="max-w-[1440px] mx-auto px-4 mt-6 pb-8">
        <div
          className="overflow-hidden rounded-2xl relative h-[260px] xs:h-[280px] sm:h-[340px] md:h-[420px]"
          onMouseEnter={stopAutoPlay}
          onMouseLeave={startAutoPlay}
        >
          <div ref={sliderRef} className="flex h-full" style={{ transform: "translateX(-100%)" }}>
              ...
              <div key={index} className="min-w-full h-full relative">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-3">
                  ...
                  <div className="bg-white/95 rounded-xl px-3 sm:px-6 py-2 sm:py-4 w-full max-w-[260px] sm:max-w-[380px] shadow-lg">
                    <p className="text-gray-400 text-[8px] sm:text-xs uppercase">Starts from</p>
                    <p className="text-lg sm:text-2xl font-bold text-gray-900">
                      ₹{banner.price?.toLocaleString("en-IN")}
                    </p>
                    <p className="text-gray-400 text-[8px] sm:text-xs mb-2">per person</p>
                    <div className="flex flex-wrap justify-center gap-1 text-[8px] sm:text-xs text-gray-500 mb-2">
                      {banner.features?.map((f, i) => <span key={i}>{f}</span>)}
                    </div>
                    {/* ✅ /tours/<slug> */}
                    <Link
                      href={`/tours/${banner.slug}`}
          ...
          <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1">
            {banners.map((_, i) => (
              <button key={i} onClick={() => goToDot(i)} aria-label={`Slide ${i + 1}`}
                className={`h-1.5 rounded-full transition-all ${i === dotIndex ? "w-5 bg-yellow-400" : "w-2 bg-white/60"}`}
              />
            ))}
          </div>
        </div>
