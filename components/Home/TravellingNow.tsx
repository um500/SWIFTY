    ...
    <div className="max-w-[1440px] mx-auto px-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
        <div>
          ...
          <Link href="/india/all" className="vw-btn-outline text-xs py-2 px-6 inline-flex">Explore Upcoming Tours</Link>
        </div>
        <div className="grid grid-cols-2 gap-3">
          {travellingNowData.map((item, i) => (
            <div key={i} className="relative rounded-xl overflow-hidden h-[180px] group cursor-pointer">
              ...
              <div className="absolute top-3 left-3 flex items-center gap-1.5">
                ...
                <span className="bg-vw-green text-white text-[10px] font-bold px-2 py-0.5 rounded">{item.count} TOURS</span>
                <span className="text-white text-[10px]">in <span className="font-semibold underline">{item.dest}</span> right now</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex items-center gap-4 mt-6 p-4 bg-vw-light-bg rounded-xl">
        <div className="w-10 h-10 rounded-full bg-vw-green text-white flex items-center justify-center font-bold text-sm flex-shrink-0">S</div>
        <div>
          <p className="text-sm text-vw-text italic">"Awesome experience, well managed tour!"</p>
          <p className="text-xs text-vw-text-light mt-0.5">– Subodh Janardan N. · <span className="font-semibold">Shimla Manali</span></p>
        </div>
      </div>
    </div>
  </section>
