...
// Build a human-readable heading from active slug(s)
function buildLabel(areaSlug: string, stateSlug: string, countrySlug: string, filterLabel: string): string {
  if (filterLabel) return filterLabel;
  const raw = areaSlug || stateSlug || countrySlug;
  if (!raw) return "";
...
export default function ToursClient({
  ...
  // Re-sync when server sends new tours (URL changed)
  useEffect(() => {
    setFiltered(tours);
  }, [tours]);

  const sorted = useMemo(() => sortTours(filtered, sortBy), [filtered, sortBy]);

  const handleFilterChange = (f: any[]) => setFiltered(f);
  const handleSort = (val: string) => setSortBy(val);

  const label = buildLabel(areaSlug, stateSlug, countrySlug, filterLabel);

  const sidebarProps = { tours, areaSlug, stateSlug, countrySlug, specialSlug, customSlug, onFilterChange: handleFilterChange };
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="mb-5">
        ...
        <p className="text-sm text-gray-500 mt-0.5">
          Showing 1–{sorted.length} of {tours.length} packages
        </p>
      </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden">
            <button
              ...
              className={`p-1.5 transition-colors ${view === "list" ? "bg-blue-500 text-white" : "text-gray-400 hover:text-gray-700"}`}
            >
              <AlignJustify size={14} />
            </button>
            <button
              onClick={() => setView("grid")}
              className={`p-1.5 transition-colors ${view === "grid" ? "bg-blue-500 text-white" : "text-gray-400 hover:text-gray-700"}`}
            >
              <LayoutGrid size={14} />
            </button>
          </div>
        ...
        <div className="flex-1 min-w-0">
          {sorted.length === 0 ? (
            ...
            <div className="flex flex-col gap-4">
              {sorted.map((tour) => <TourCard key={tour._id} tour={tour} view="list" />)}
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {sorted.map((tour) => <TourCard key={tour._id} tour={tour} view="grid" />)}
            </div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex lg:hidden">
          ...
          <div className="relative ml-auto w-[85%] max-w-sm h-full bg-white overflow-y-auto shadow-xl">
            <div className="flex items-center justify-between px-4 py-3 border-b">
              ...
              <button onClick={() => setMobileFilterOpen(false)}><X size={20} className="text-gray-500" /></button>
            </div>
            <div className="p-4">
              <FilterSidebar {...sidebarProps} onFilterChange={(f) => { handleFilterChange(f); setMobileFilterOpen(false); }} />
            </div>
          </div>
        </div>
      )}
